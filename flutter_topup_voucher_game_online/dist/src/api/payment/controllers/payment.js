...
exports.default = strapi_1.factories.createCoreController('api::payment.payment', ({ strapi }) => ({
    async midtransCallback(ctx) {
        try {
            ...
            const { order_id, transaction_status, status_code, gross_amount, signature_key, payment_type, transaction_time, } = ctx.request.body;
            ...
            console.log(`✅ Payment updated: ${order_id} | ${previousStatus} → ${newStatus} | Method: ${payment_type} → ${transaction_status}`);
            return ctx.send({
                success: true,
